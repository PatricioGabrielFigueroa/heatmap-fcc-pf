...
function HeatMap() {
  ...
  useEffect(() => {
    ...
    const palette = d3.select(paletteRef.current).append('svg').attr('height', heightP).attr('width', widthP); //.attr('x', 1000) IS NOT APPLICABLE

    const paletteG = palette.append('g')
    .attr('transform', `translate(${marginP.left}, ${marginP.top})`);

    const colorPalette = paletteG.selectAll('rect').data(valueScheme)
    .enter().append('rect')
    .attr('x', d => xScaleP(d))
    ...
    if (data.length > 0 && fetchedData === true) {
      ...
      const baseMinusVariance = data.map(d => {
        d.result = 8.66 + d.variance; // Create 'd.result' within the state with dot notation
      });
    ...
    const titles = enter.append('text')
    .text('Monthly Global Land-Surface Temperature')
    .attr('transform', `translate(${innerWidth / 4 - 40}, -60)`)
    .attr('font-size', 30);

    const titles2 = enter.append('text')
